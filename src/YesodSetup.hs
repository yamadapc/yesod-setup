...
module YesodSetup (defaultMain) where
...
instance FromJSON DatabaseSettings where
    parseJSON (Object o) =
        DatabaseSettings <$> (dropStart <$> (o .: "host")) <*>
                             (dropStart <$> (o .: "port")) <*>
                             (dropStart <$> (o .: "user")) <*>
                             (dropStart <$> (o .: "password")) <*>
                             (dropStart <$> (o .: "database"))
      where
        dropStart s = let s' = Text.dropWhile (/= ':') s
                      in if Text.null s'
                         then s
...
runSetup :: DatabaseSettings -> IO ()
runSetup db = do
    Text.putStrLn ("Connecting to " <> dsHost db <> "...")
    conn <-
        connectPostgreSQL
            (Text.encodeUtf8
                 ("host='" <> dsHost db <> "' " <> "port=" <> dsPort db <> "' " <>
                  "user='postgres'" <>
                  "password='postgres'"))
    -- <> "dbname='postgres'"))
    ...
    _ <-
        execute_
            conn
            (fromString (Text.unpack ("create role \"" <> dsUser db <> "\""))) `catch`
        handleSqlError
    _ <-
        execute_
            conn
            (fromString
                 (Text.unpack ("alter role \"" <> dsUser db <> "\" with login"))) `catch`
        handleSqlError
    ...
    _ <-
        execute_
            conn
            (fromString
                 (Text.unpack ("create database \"" <> dsName db <> "\""))) `catch`
        handleSqlError
    ...
    _ <-
        execute_
            ...
            (fromString
                 (Text.unpack
                      ("alter role \"" <> dsUser db <> "\" with password '" <>
                       dsPassword db <>
                       "'"))) `catch`
        handleSqlError
    ...
    _ <-
        execute_
            ...
            (fromString
                 (Text.unpack
                      ("grant all privileges on database \"" <> dsName db <>
                       "\" to \"" <>
                       dsUser db <>
                       "\""))) `catch`
        handleSqlError
...
defaultMain = do
    ...
    mvalue <-
        Yaml.decodeFile (pr </> "config/settings.yml") :: IO (Maybe Value)
    case mvalue of
        Just (Object h) ->
            case HashMap.lookup "database" h of
                Just database -> do
                    mdb <-
                        Yaml.parseMonad parseJSON database :: IO (Maybe DatabaseSettings)
                    case mdb of
                        Nothing -> do
